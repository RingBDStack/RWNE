...
def eval_online(options):
    ...
    time_start = time.time()
    logger.info('\t reading labeled data from file {}'.format(options.label_path))
    id_list_totoal, labels_list_total = utils.get_labeled_data(options.label_path)
    logger.info('\t reading labeled data completed in {}s'.format(time.time() - time_start))
...
if __name__ == '__main__':
    y_true = np.array([[0,0,1],[0,1,0],[1,0,1]])
    y_pred = np.array([[1,0,1],[0,1,0],[0,1,1]])

    macro = f1_score(y_true, y_pred, average="macro")
    micro = f1_score(y_true, y_pred, average="micro")
    print("{},{}".format(macro,micro))
    macro, micro = eval_utils.f1_scores_multilabel(y_true, y_pred)
    print("{},{}".format(macro, micro))
