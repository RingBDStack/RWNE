...
def _eval(eval_graph, except_graph):
    ...
    for i in range(np.size(similarity, axis=0)):
        similarity[i,i] = 0
    precisionK_list = eval_utils.check_precK(nodeID_list, similarity, max_index=PREC_K, eval_graph=eval_graph, except_graph=except_graph)
    MAP = eval_utils.check_MAP(nodeID_list, similarity, eval_graph=eval_graph, except_graph=except_graph)
...
def _sample_thread_body(repeated_times):
    ...
    for _ in range(repeated_times):
        ...
        net_sample = net_eval.sample_by_nodes(SAMPLE_NODES, rule = SAMPLE_RULE)
        logger.info('sampling nodes completed in {}s'.format(time.time() - time_start))
        ret =_eval(net_sample, net_except)
        ret_list.append(ret)
    return ret_list
...
def eval_once(options):
    ...
    if not utils.check_rebuild(options.link_prediction_path, descrip='link_prediction', 
    ...
    net_eval = network.construct_network(data_path=options.eval_data_path, data_format=options.data_format,
    ...
    net_except = network.construct_network(data_path=options.except_data_path, data_format=options.data_format,
                                         print_net_info=False, isdirected=options.isdirected)
...
def eval_online(options):
    ...
    fr_total.write("except_net_nodes_size = {}\n".format(except_net_nodes_size))
    fr_total.write("except_net_edges_size = {}\n".format(except_net_edges_size))
    ...
    while (options.eval_online):
        ...
        logger.info('eval case: ret_list completed in {}s.\n================================='.format(time.time() - time_start))
