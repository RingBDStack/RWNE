...
def get_labeled_data(label_filepath, multilabel_rule = "all"):
    """
...
def get_vectors(wv, id_list, label_list = None):
    # vec_list = []
    # vec_label_list = []
    vector_list = []
    vector_label_list = []
    if label_list == None:
        for i in range(len(id_list)):
            ...
            if word not in wv.vocab:
                logger.warning('word \'%s\' not in vocabulary, skiped.'%word)
            else:
                vec = np.reshape(wv.word_vec(word), (1, -1))
                vector_list.append(vec)
        return np.concatenate(vector_list,axis=0)
    else:
        for i in range(len(id_list)):
            word = str(id_list[i])
            if word not in wv.vocab:
                ...
                # vec_list.append()
                # vec_label_list.append(label_list[i])
        return np.concatenate(vector_list,axis=0), vector_label_list
...
def save_word2vec_format_and_ckpt(vector_path, vectors, checkpoint_path, sess, saver, global_step_value):
    ...
    reading_reconstruction = vector_path + ".reading_reconstruction"
    ...
    time.sleep(30)
    while os.path.exists(reading_classify) or os.path.exists(reading_cluster) or os.path.exists(reading_visualization) or os.path.exists(reading_reconstruction) or os.path.exists(reading_link_prediction):
        time.sleep(5)
