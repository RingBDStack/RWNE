...
def eval_once(options):
    ...
    logger.info('\t save_path: {}'.format(options.visualization_path))
    ...
    time_start = time.time()
    id_list, labels_list = utils.get_labeled_data(options.label_path, multilabel_rule=options.multilabel_rule)
    features_matrix, labels_list = utils.get_vectors(utils.get_KeyedVectors(options.vectors_path), id_list, labels_list)
...
def eval_online(options):
    ...
    # print logger
    logger.info('eval case: visualization...')
    logger.info('\t save_dir: {}'.format(visual_dir))
    logger.info('\t method: t-SNE')
    logger.info('\t multilabel_rule: {}'.format(options.multilabel_rule))
    logger.info('\t marker_size: {}'.format(options.marker_size))
    logger.info('\t eval_online: {}'.format(options.eval_online))
    ...
    reading = options.vectors_path + ".reading_visualization"
    ...
    while (options.eval_online):
        ...
        features_matrix, labels_list = utils.get_vectors(utils.get_KeyedVectors(options.vectors_path),
                                                         id_list, labels_list)
